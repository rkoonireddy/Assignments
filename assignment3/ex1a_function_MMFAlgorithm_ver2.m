function [ret, delta_mat, gamma_mat, mu_mat, Sigma_mat, nu_vec, x_mat] = ex1a_function_MMFAlgorithm_ver2(x_mat, true_df, initial_df, reps, dim, n_samp, wgts)
...
% % % add in each step eye(d) to sigma to prevent that sigma becomes singular
Sigma_mat(:,:,1) = ( x_mat - repmat(mu_mat(:,1),1,n_samp) ) * ( x_mat - repmat(mu_mat(:,1),1,n_samp) )'/n_samp;%+eye(dim);
delta_mat = zeros(n_samp, reps);
gamma_mat = zeros(n_samp, reps);

% e-step for r = 0 (or rather r = 1 since matlab uses 1-indexing)
%delta_mat(:,1) = sum( ((Sigma_mat(:,:,1)^(-1))*(x_mat - repmat(mu_mat(:,1), 1, n_samp))) .* (x_mat - repmat(mu_mat(:,1), 1, n_samp)), 1);
%for i = 1:n_samp
%    delta_mat(i, 1) = ( x_mat(:,i) - mu_mat(:,1) )' / Sigma_mat(:,:,1) * ( x_mat(:,i) - mu_mat(:,1) );
%    gamma_mat(i, 1) = ( nu_vec(1) + dim ) / ( nu_vec(1) + delta_mat(i, 1) );
%end
...
% % m-step
    ...
    res = fzero(@(x)  Phi_func( x / 2 ) - Phi_func( (x + dim) / 2 ) + temp_sum, [10^-10, 10^15]);
    disp(res);
    nu_vec(r+1) = res; % if using syms: since solution is unique (o.w. try subs() )
end
